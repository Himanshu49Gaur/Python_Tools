...
class EulerApp(QMainWindow):
    """
    A PyQt5 GUI application for solving first-order ODEs using the
    standard Euler Method, with detailed step-by-step explanations.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Euler Method Solver")
    ...
    def create_input_widgets(self):
        ...
        self.solve_button = QPushButton("Solve with Euler's Method")
    ...
    def run_solver(self):
        """Main function to trigger the Euler method and display detailed steps."""
        self.solution_display.clear()
        ...
        self.solution_display.append("--- Starting Euler's Method ---\n")
        ...
        for i in range(num_steps):
            ...
            # --- Slope Calculation ---
            self.solution_display.append("1. Calculate the slope at the current point:")
            self.solution_display.append("   Formula: slope = f(x_n, y_n)")
            
            slope = f(x_current, y_current)
            
            self.solution_display.append(f"   slope = f({x_current:.4f}, {y_current:.4f}) = {slope:.6f}\n")

            # --- Next Y Value Calculation ---
            self.solution_display.append("2. Calculate the next value y_{n+1} using the Euler formula:")
            self.solution_display.append("   Formula: y_{n+1} = y_n + h * slope")
            
            y_next = y_current + h * slope
            
            self.solution_display.append(f"   y_{i+1} = {y_current:.6f} + {h:.4f} * {slope:.6f}")
            self.solution_display.append(f"   y_{i+1} = {y_next:.8f}")
