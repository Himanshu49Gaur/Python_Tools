...
class FixedPointApp(QMainWindow):
    """
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Fixed-Point Iteration Solver")
    ...
    def update_ui_size(self):
        ...
        # Pre-fill with an example for n=2
        if n == 2:
            if self.equations_table.item(0, 0) is None or self.equations_table.item(0,0).text() == "":
                self.equations_table.setItem(0, 0, QTableWidgetItem("(x1**2 + x2**2 + 8) / 10"))
                self.equations_table.setItem(1, 0, QTableWidgetItem("(x1*x2**2 + x1 + 8) / 10"))
                self.guess_table.setItem(0, 0, QTableWidgetItem("1.5"))
                self.guess_table.setItem(0, 1, QTableWidgetItem("1.5"))
    ...
    def run_solver(self):
        """Main function to trigger the Fixed-Point Iteration method."""
        self.solution_display.clear()
        ...
        self.solution_display.append("--- Starting Fixed-Point Iteration Solver ---\n")

        # --- Convergence Check at Initial Point ---
        try:
            ...
            if spectral_radius < 1:
                self.solution_display.append("CONDITION MET: ρ < 1. Iteration is likely to converge from this point.\n")
            else:
                self.solution_display.append("WARNING: ρ >= 1. Iteration is likely to DIVERGE from this point.\n")
        except Exception as e:
        ...
        for k in range(max_iter):
            ...
            self.solution_display.append(self.format_vector(x, title=f"Current Guess x_({k})"))
            
            # Apply the iteration function: x_new = g(x)
            try:
                x_new = np.array([g_func(*x) for g_func in G_numeric_list])
            ...
            self.solution_display.append(self.format_vector(x_new, title=f"Next Value x_({k+1}) = g(x_({k}))"))
